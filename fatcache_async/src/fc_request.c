...
//extern FILE *Set_Latency;
extern FILE *In_Rate;

/********=FOR INCOMING RATE=**********/
#define in_num_interval 10000
int in_num = 0;
struct timeval in_start, in_finish, in_diff;
double in_rate;
...
req_process(struct context *ctx, struct conn *conn, struct msg *msg)
{
    ...
    if(in_num == 0)
    {
	gettimeofday(&in_start, 0);
    }
    in_num++;
    if(in_num == in_num_interval)
    {
	gettimeofday(&in_finish, 0);
    	timeval_subtract(&in_diff, &in_start, &in_finish);
    	fprintf(In_Rate, "%f %f\n", (double)in_diff.tv_sec, (double)in_diff.tv_usec);
	in_num = 0;
    }
