...
/*szy: for the lock*/
extern pthread_mutex_t mut_IX;
extern pthread_mutex_t mut_I;

extern struct slabinfo *stable;
extern struct slabclass *ctable; 
...
itemx_put(struct itemx *itx)
{
    ...
    pthread_mutex_lock(&mut_IX);
    nfree_itemxq++;
    STAILQ_INSERT_HEAD(&free_itemxq, itx, tqe);
    pthread_mutex_unlock(&mut_IX);
}
...
itemx_removex(uint32_t hash, uint8_t *md)
{
	...
	bucket = itemx_bucket(hash);
	//pthread_spin_lock(&mut_IX);
	nitx--;
	//pthread_spin_unlock(&mut_I);
