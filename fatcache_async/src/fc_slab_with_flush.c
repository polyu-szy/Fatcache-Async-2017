...
struct free_slab_entry *FQ_InsertSort(struct free_slab_entry *head, int num)
{
	...
	struct free_slab_entry *temp = NULL;
	...
	if (n_sinfo->erase_count > c_sinfo->erase_count)
	{
...
slab_evict(void)
{
    ...
    ****************************************************************/
	...
	fprintf(LRU_fp, "Erase Option, Addr: %d, count %d, valid ratio %f\n", sinfo->addr, sinfo->erase_count, sinfo->valid_ratio);
	...
	//printf("The number of valid slots in this slab is %d\n", valid_number);
    log_debug(LOG_DEBUG, "evict slab at disk (sid %"PRIu32", addr %"PRIu32")",
              sinfo->sid, sinfo->addr);

	//printf("The valid ratio of chosen slab is %f\n", sinfo->valid_ratio);
...
_slab_drain(void)
{
    ...
    ****************************************************************/
    ...
    Alloc_sid = FQ_deleteHead(Channel_Alloc_Order); //free disk slab
    ...
    ****************************************************************/
    ...
    //add the slab to valid ratio full disk slab queue
...
slab_get_item(uint8_t cid)
{
    ...
    if(full_memory_slab_num >= FLUSH_START_THRESHOLD)
    {
    ...
    while(full_memory_slab_num >= FLUSH_SYNC_THRESHOLD)
...
void *flush_thread_function(void *arg)
{
	...
	while(1)
	{
		...
		//printf("Erase done, the number of free slabs now is %d!\n", nfree_dsinfoq);
		if(full_memory_slab_num <= FLUSH_STOP_THRESHOLD)
		{
			pthread_mutex_lock(&fmut[ch]);
...
{
	if((FLUSH_STATUS[ch] == STOP) && (full_memory_slab_num >= FLUSH_STOP_THRESHOLD))
	{
		pthread_mutex_lock(&fmut[ch]);
		FLUSH_STATUS[ch] = RUN;
...
slab_init(void)
{
   ...
   GC_STATUS = STOP;
    /*Init the queues used in current program*/
    int m;
    ...
    mend = mstart + mspace;
	//FLUSH_START_THRESHOLD = (int)(0.7 * nmslab);
	//FLUSH_STOP_THRESHOLD = (int)(0.5 * nmslab);
	//FLUSH_SYNC_THRESHOLD = (int)(0.85 * nmslab);
	FLUSH_START_THRESHOLD = 3;
	FLUSH_STOP_THRESHOLD = 1;
	FLUSH_SYNC_THRESHOLD = 4;
    ...
    ****************************************************************/
	...
	highWatermark = (int)(0.2 * ndslab);
	lowWatermark = (int)(0.05 * ndslab);
